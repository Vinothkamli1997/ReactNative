...
const SampleLoginScreen = () => {
  ...
  const checkNumber = text => {
    ...
    // AsyncStorage.setItem('storedNumber', number);
  };

  useEffect(() => {
    console.log('name', name);
    console.log('Number', number);
    // Update AsyncStorage here if needed
  }, [name, number]);
  ...
  const checkConformBtn = number => {
    ...
    if (!name.trim()) {
      ...
      Alert.alert('Specail Character Not Allowed');
    } else if (!/^\d{0,10}$/.test(number)) {
      Alert.alert('Enter a valid mobile number without special characters');
    } else if (number.length === 10) {
      AsyncStorage.setItem('storedName', name);
      AsyncStorage.setItem('storedNumber', number);

      navigation.navigate('SampleOtpScreen', {number: number});
